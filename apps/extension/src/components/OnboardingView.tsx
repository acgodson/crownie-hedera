...
const OnboardingView: React.FC<OnboardingViewProps> = ({ onComplete }) => {
  ...
  // Save onboarding state when values change
  useEffect(() => {
    ...
    if (currentStep > 0 || privateKey || accountId) {
      saveOnboardingState()
    }
  }, [currentStep, privateKey, accountId, network])

  const steps = [
    {
      icon: ExternalLink,
      title: 'Get Hedera Account',
      description: 'Create an account on Hedera Developer Portal with 1000 HBAR funding'
    },
    {
      icon: Key,
      ...
      <div className="w-full max-w-80 h-[500px] text-white flex flex-col items-center justify-center p-6"
        ...
        <div className="text-center space-y-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${
            initStatus.success ? 'bg-green-500/20' : 'bg-red-500/20'
          }`}>
            {initStatus.success ? (
      ...
      <div className="flex-1 flex flex-col text-center overflow-y-auto">
        ...
        {currentStep === steps.length - 1 ? (
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-full font-semibold text-black"
