...
export class Wallet {
    ...
    // Removed signOrder method - not needed for local swaps

    /**
     * Fund wallet with testnet tokens (for real testnet usage)
     */
    public async fundWithTestnetTokens(tokens: {address: string, amount: bigint}[]): Promise<void> {
        for (const token of tokens) {
            // For testnet, tokens should be available from faucet
            // This method can be extended to interact with faucet APIs
            console.log(`Please fund ${await this.getAddress()} with ${token.amount} of token ${token.address}`)
        }
    }

    /**
     * Deposit tokens into a resolver contract for escrow operations
     */
    public async depositToResolver(resolverAddress: string, token: string, amount: bigint): Promise<void> {
        // First approve the resolver to spend tokens
        await this.unlimitedApprove(token, resolverAddress)
        
        // The actual deposit happens when creating orders in the resolver
        console.log(`Approved resolver ${resolverAddress} to spend ${amount} of token ${token}`)
    }
