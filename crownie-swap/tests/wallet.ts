...
export class Wallet {
  ...
  public async fundWithTestnetTokens(
    tokens: { address: string; amount: bigint }[]
  ): Promise<void> {
    for (const token of tokens) {
      console.log(
        `Please fund ${await this.getAddress()} with ${token.amount} of token ${
          token.address
        }`
      );
    }
  ...
  public async depositToResolver(
    ...
    console.log(
      `Approved resolver ${resolverAddress} to spend ${amount} of token ${token}`
    );
  }
